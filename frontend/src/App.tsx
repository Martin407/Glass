...
function App() {
  ...
  const handleCreateSession = async () => {
    try {
      const res = await agentsApi.createSession({ agent: 'agent_id_placeholder' });
      if (res.id) {
        ...
        <header className="h-14 border-b border-gray-200 flex items-center px-6 justify-between flex-shrink-0">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="text-sm font-medium bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-md flex items-center gap-1 shadow-sm transition-colors"
            >
